...
router.get("/dashboard/summary", async (_req, res) => {
  ...
  const [{ totalSpent }] = await db
    ...
    .from(expensesTable)
    .where(sql`to_char(${expensesTable.date}::date, 'YYYY-MM') = ${month}`);
  ...
  const unlockedAch = await db
    ...
    .where(eq(achievementsTable.isUnlocked, true));
  ...
  res.json({
    ...
    recentExpenses: recentRows.map((r) => ({
      ...r,
...
router.get("/dashboard/health-score", async (_req, res) => {
  ...
  const spendingConsistency = 70; // static baseline for now
  const score = Math.round(
    budgetAdherence * 0.35 +
    savingsRate * 0.25 +
    goalAchievement * 100 * 0.2 +
...
router.get("/dashboard/spending-by-category", async (req, res) => {
  ...
  const catRows = await db
    .select({
      category: expensesTable.category,
      amount: sql<string>`sum(${expensesTable.amount})`,
    })
    .from(expensesTable)
    .where(sql`to_char(${expensesTable.date}::date, 'YYYY-MM') = ${month}`)
    .groupBy(expensesTable.category)
  ...
  const result = catRows.map((c) => {
    ...
    return {
      category: c.category,
      amount: amt,
      percentage: totalAmt > 0 ? Math.round((amt / totalAmt) * 1000) / 10 : 0,
      budget: budgetAmt,
      isOverBudget: budgetAmt > 0 && amt > budgetAmt,
    };
  });
...
router.get("/dashboard/monthly-trend", async (_req, res) => {
  const rows = await db
    .select({
      ...
      totalSpent: sql<string>`sum(${expensesTable.amount})`,
    })
    .from(expensesTable)
  ...
  const budgetRows = await db
    .select({
      ...
      totalBudget: sql<string>`sum(${budgetsTable.monthlyLimit})`,
    })
    .from(budgetsTable)
  ...
  const result = rows.reverse().map((r) => ({
    ...
    totalBudget: budgetMap.get(r.month) ?? 0,
    totalSaved: 0,
  }));
...
router.get("/dashboard/forecast", async (_req, res) => {
  ...
  res.json({
    predictedMonthlySpend: Math.round(predictedMonthlySpend * 100) / 100,
    currentMonthSpend: Math.round(spent * 100) / 100,
    daysLeft,
  ...
  for (const b of budgets) {
    ...
    if (pct >= 1) {
      alerts.push({
        ...
        message: `You have exceeded your ${b.category} budget by RM${(spentAmt - limit).toFixed(2)}.`,
        severity: "danger",
        category: b.category,
      });
    } else if (pct >= 0.8) {
      alerts.push({
        id: `budget-warn-${b.id}`,
        type: "budget_warning",
        message: `You have used ${Math.round(pct * 100)}% of your ${b.category} budget.`,
        severity: "warning",
        category: b.category,
      });
    }
