...
router.get("/challenges", async (_req, res) => {
  const rows = await db.select().from(challengesTable).orderBy(challengesTable.createdAt);
...
router.patch("/challenges/:id", async (req, res) => {
  ...
  const [row] = await db.update(challengesTable).set(vals).where(eq(challengesTable.id, id)).returning();
  if (!row) {
